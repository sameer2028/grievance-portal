  ...
  <div className="animate-fade-in">
    ...
    <section className="bg-gradient-to-br from-primary-700 via-primary-600 to-primary-800 text-white">
      <div className="max-w-5xl mx-auto px-6 py-24 text-center">
        <span className="inline-block mb-4 px-3 py-1 bg-primary-500/40 rounded-full text-xs font-semibold uppercase tracking-widest">
          Smart India Hackathon 2024
        </span>
        <h1 className="text-4xl sm:text-5xl font-bold leading-tight mb-6 text-balance">
          AI-Powered Public<br />Grievance Portal
        </h1>
        <p className="text-primary-100 text-lg max-w-2xl mx-auto mb-10 text-balance">
        ...
        <div className="flex flex-wrap gap-4 justify-center">
          <Link to="/register" className="btn bg-white text-primary-700 hover:bg-primary-50 px-8 py-3 font-semibold shadow-lg">
            Submit a Grievance
          </Link>
          <Link to="/track" className="btn border border-white/40 text-white hover:bg-white/10 px-8 py-3 font-semibold">
            Track Your Ticket
          </Link>
        </div>
      </div>
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-8">
          {STEPS.map((s, i) => (
            <div key={s.num} className="text-center">
              <div className="w-12 h-12 rounded-full bg-primary-600 text-white font-bold text-sm flex items-center justify-center mx-auto mb-4">
                {s.num}
              </div>
              {i < STEPS.length - 1 && (
                <div className="hidden lg:block absolute translate-x-full translate-y-[-2rem] text-gray-300 text-2xl ml-4">→</div>
              )}
    ...
    <section className="bg-primary-600 text-white py-16 text-center px-6">
      <h2 className="text-3xl font-bold mb-4">Ready to raise your voice?</h2>
      <p className="text-primary-100 mb-8 max-w-lg mx-auto">
        It only takes 2 minutes. No paperwork. No queues.
      </p>
      <Link to="/register" className="btn bg-white text-primary-700 hover:bg-primary-50 px-10 py-3 font-semibold shadow-lg">
