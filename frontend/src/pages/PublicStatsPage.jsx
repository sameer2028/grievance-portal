...
const StatBlock = ({ value, label, sub }) => (
  <div className="text-center">
    <p className="text-3xl sm:text-4xl font-black text-primary-600 dark:text-primary-400">{value ?? '—'}</p>
    <p className="text-sm font-semibold text-gray-800 dark:text-gray-200 mt-1">{label}</p>
...
const PublicStatsPage = () => {
  ...
  return (
    <div className="animate-fade-in">
      {/* Hero */}
      <section className="bg-primary-800 dark:bg-gray-900 text-white py-16 px-6 text-center">
      ...
      <section className="bg-gray-50 dark:bg-gray-900/50 py-12 px-6">
        <div className="max-w-4xl mx-auto">
          <h2 className="text-xl font-bold text-gray-900 dark:text-white mb-6 text-center">Department Performance</h2>
              ...
              return (
                <div key={d._id} className="card">
                  <div className="flex justify-between items-start mb-3">
                    ...
                    <span className="text-xs text-gray-500 dark:text-gray-400">{d.count} total</span>
                  </div>
                  <div className="flex items-center gap-3">
                    ...
                    <span className="text-sm font-bold text-primary-600 dark:text-primary-400 w-12 text-right">{rate}%</span>
                  </div>
                  <p className="text-xs text-gray-400 dark:text-gray-500 mt-1">{d.resolved} resolved</p>
                </div>
              );
            })}
          </div>
      ...
      {byDistrict?.length > 0 && (
        <section className="max-w-4xl mx-auto px-6 py-12">
          <h2 className="text-xl font-bold text-gray-900 dark:text-white mb-6 text-center">Top Districts by Grievances</h2>
          <div className="card">
            {byDistrict.map((d, i) => {
              const max = byDistrict[0].count;
              return (
                <div key={d._id} className={`flex items-center gap-4 py-3 ${i !== 0 ? 'border-t border-gray-100 dark:border-gray-700' : ''}`}>
                  <span className="w-5 text-sm font-bold text-gray-400 dark:text-gray-500 shrink-0">{i + 1}</span>
                  <span className="text-sm font-medium text-gray-800 dark:text-gray-200 w-32 shrink-0">{d._id}</span>
                  ...
                  <span className="text-xs text-gray-500 dark:text-gray-400 w-16 text-right shrink-0">{d.count} cases</span>
                </div>
              );
            })}
          </div>
      ...
      {/* CTA */}
      <section className="bg-primary-600 dark:bg-gray-900 dark:border-t dark:border-gray-800 text-white py-12 text-center px-6">
